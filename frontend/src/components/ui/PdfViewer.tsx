...
export function PdfViewer({ fileUrl, title, className, onPageCountLoaded }: Readonly<PdfViewerProps>) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // Fetch PDF as ArrayBuffer in the main thread to avoid worker cross-origin CORS failures
        ...
        // Fallback: let the worker attempt the URL directly (works for public CDN URLs)
        if (!isMounted) return;
        loadingTask = pdfjsLib.getDocument({
          url: fileUrl,
          cMapUrl: 'https://cdn.jsdelivr.net/npm/pdfjs-dist@6.3.289/cmaps/',
      ...
      if (!loadingTask || !isMounted) return;
