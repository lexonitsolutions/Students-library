...
import { Bell, History, Menu, Moon, Settings, Sparkles, Sun, User } from 'lucide-react';
      ...
      <div className="relative ml-auto flex shrink-0 items-center gap-1 sm:gap-2">
        {!isExploring && user && (
          <NavLink
            to="/ai-learning"
            className={({ isActive }) =>
              `hidden md:inline-flex h-10 shrink-0 items-center gap-2 rounded-xl px-3 text-sm font-semibold transition-colors focus-visible:outline-2 focus-visible:outline-primary ${
                isActive
        ...
        {/* Theme toggle (tablet and desktop) - 2-state Light / Dark */}
        <button
          onClick={cycleTheme}
          aria-label="Toggle theme"
          title={theme === 'dark' ? 'Switch to Light Mode' : 'Switch to Dark Mode'}
          className="hidden h-8 w-14 items-center rounded-full border border-card-border bg-surface-container p-0.5 transition-all duration-200 hover:border-primary/50 md:flex relative cursor-pointer"
        >
          <span
