...
export const getMode = (id: LearningMode) => learningModes.find(mode => mode.id === id) ?? learningModes[0];
...
export function parseLearningReply(value: unknown): LearningReply {
  if (!value || typeof value !== 'object' || !('text' in value) || typeof value.text !== 'string' || !value.text.trim() || value.text.length > 30000) throw new Error('The learning assistant returned an invalid response. Please retry.');
  ...
  return { text: value.text, quiz, flashcards };
}
