...
export function MaterialDetailsPage() {
  ...
  useEffect(() => {
    if (!id || material?.id !== id || !user?.id || isExploring) return;
    let active = true;

    // Database uniqueness makes revisits and React Strict Mode safe, while
    // recording every visit keeps recent activity timestamps current.
    incrementViews(id, user.id)
      .then((views) => {
        if (active) setMaterial((prev) => prev?.id === id ? { ...prev, views } : prev);
      })
      .catch((err) => console.warn('Failed to record material view:', err));

    return () => { active = false; };
  }, [id, material?.id, user?.id, isExploring]);
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-12 items-start">
          ...
          <div className="lg:col-span-4 flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-4 rounded-2xl border border-card-border bg-surface-container-low p-5 shadow-card">
              ...
              <div className="flex items-center justify-center gap-1.5 text-xs text-on-surface-variant" title="Unique signed-in viewers">
                <Eye size={15} />
