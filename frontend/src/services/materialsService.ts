...
/** Record one unique viewer; the database owns deduplication and the counter. */
export async function incrementViews(id: string, userId: string): Promise<number> {
  if (!userId) throw new Error('A signed-in viewer is required');
  ...
  // Read the persisted count; never invent a count or perform a racing client update.
