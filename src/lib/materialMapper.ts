...
export function toMaterial(row: MaterialRow, uploader: PublicProfileRow | undefined, isSaved = false, isLiked = false): Material {
  ...
  return {
    ...
    likes: Math.max(
      Number((row as any).likes_count ?? 0),
      Number((row as any).likes ?? 0),
    ...
    college: row.college ?? row.university ?? undefined,
    branch: row.branch ?? undefined,
    year: row.year ?? undefined,
    course: (row as any).course ?? (row.branch?.includes(' - ') ? row.branch.split(' - ')[0] : undefined),
    isSaved,
    isLiked,
