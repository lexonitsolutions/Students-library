...
import { listLeaderboardForUI, type LeaderboardEntry } from '../services/materialsService';
    ...
    <div className="flex flex-col gap-8 pb-12">
      ...
      {loading && (
        <div className="flex flex-col items-center justify-center py-24 gap-4 text-on-surface-variant">
          ...
          <p className="text-body-md font-medium">Loading leaderboard…</p>
        </div>
      )}
      ...
      {!loading && sortedStudents.length === 0 && (
        <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
          <span className="flex h-20 w-20 items-center justify-center rounded-3xl bg-amber-500/10 text-amber-500 border border-amber-500/20">
            <Trophy size={36} />
          </span>
          <h2 className="text-headline-sm font-bold text-on-surface">No contributors yet</h2>
          <p className="text-body-md text-on-surface-variant max-w-sm">
            Be the first to upload study materials and claim the top spot on the leaderboard!
          </p>
        </div>
      )}
          ...
          <div className="flex flex-col gap-4 mt-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-label-sm font-semibold text-on-surface-variant">
                Sorted by {sortBy === 'uploads' ? 'Total Uploaded Resources' : 'Total Resource Views'}
              </span>
            </div>
                    ...
                    >
                      <div className="flex items-center gap-4 min-w-0">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="truncate text-label-sm text-on-surface-variant">
                            {student.university}{student.branch ? ` • ` : ''}<span className="text-outline">{student.branch}</span>
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-between sm:justify-end gap-6 pt-2 sm:pt-0 border-t sm:border-t-0 border-card-border">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <p className="text-body-md font-extrabold text-on-surface">{student.totalUploads} {student.uploadLabel || (student.totalUploads === 1 ? 'Upload' : 'Uploads')}</p>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <p className="text-body-md font-extrabold text-emerald-600">{student.totalViews.toLocaleString()} {student.totalViews === 1 ? 'View' : 'Views'}</p>
                          </div>
                        </div>
                      </div>
                    </motion.div>
